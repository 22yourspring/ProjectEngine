...
struct TPolynomialRootSolver
{
	 ...
	 */
	TPolynomialRootSolver(std::span<const RealType> PolyCoeffs, RealType RangeStart, RealType RangeEnd, 
	 ...
	 */
	int32 FindRootsInRange(std::span<const RealType> PolyCoeffs, RealType RangeStart, RealType RangeEnd, 
		RealType Tolerance = (RealType)UE_SMALL_NUMBER, int32 MaxNewtonIterations = 20, RealType NearRootTolerance = (RealType)UE_SMALL_NUMBER)
	{
		Roots.clear();

		if (PolyCoeffs.size() < PolynomialDegree + 1)
		{
			return 0;
		}
		...
		RealType LocalCoeffs[PolynomialDegree + 1];
		RealType DerivCoeffs[PolynomialDegree];
		...
		RealType FoundRoots[PolynomialDegree + 1];
			...
			if (LocalCoeffs[2] != 0)
			{
				...
				if (Root1 > RangeStart && Root1 < RangeEnd)
				{
					...
					if (NumFoundRoots == 2)
					{
						...
						{
							std::swap(FoundRoots[0], FoundRoots[1]);
						}
		...
		if constexpr (PolynomialDegree >= 3)
		{
			...
			for (int32 CurDegree = 3; CurDegree <= PolynomialDegree; ++CurDegree)
			{
				...
				RealType CurStartValue = EvalPolynomial(LocalCoeffs, CurDegree, CurStart);
				...
				for (int32 RootIdx = 0; RootIdx < NumFoundRoots + 1; ++RootIdx)
				{
					...
					RealType CurEndValue = EvalPolynomial(LocalCoeffs, CurDegree, CurEnd);
					...
					if (CurStartValue * CurEndValue < 0)
					{
						...
						do
						{
							RealType SearchParamValue = EvalPolynomial(LocalCoeffs, CurDegree, SearchParam);
							...
							RealType SearchParamDerivValue = EvalPolynomial(DerivCoeffs, CurDegree - 1, SearchParam);
		...
		{
			Roots.push_back(FoundRoots[Idx]);
		}
