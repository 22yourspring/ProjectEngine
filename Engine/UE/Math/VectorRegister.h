...
#include "UE/Math/UnrealMathNeon.h"
#elif defined(__cplusplus_cli)
#include "UE/Math/UnrealMathFPU.h" // there are compile issues with UnrealMathSSE in managed mode, so use the FPU version
#elif PLATFORM_ENABLE_VECTORINTRINSICS
#include "UE/Math/UnrealMathSSE.h"
#else
#include "UE/Math/UnrealMathFPU.h"
#endif
...
{
	// Have to do this with partial specializations to work around Clang bug on Windows
	template<typename T>
	struct TVectorRegisterTypeHelper
	{
		using Type = void;
	};

	template<>
	struct TVectorRegisterTypeHelper<float>
	{
		using Type = VectorRegister4Float;
	};

	template<>
	struct TVectorRegisterTypeHelper<double>
	{
...
#include "UE/Math/UnrealMathVectorCommon.h.inl"
...
struct alignas(16) PersistentVectorRegister4Double
{
	...
	UE_FORCEINLINE_HINT PersistentVectorRegister4Double() = default;

	UE_FORCEINLINE_HINT PersistentVectorRegister4Double(const VectorRegister4Double& Register)
	{
		VectorStore(Register, XYZW);
	}

	UE_FORCEINLINE_HINT PersistentVectorRegister4Double(const VectorRegister4Float& Register)
	{
		VectorStore(VectorRegister4Double(Register), XYZW);
	}

	inline PersistentVectorRegister4Double& operator=(const VectorRegister4Double& Register)
	{
		VectorStore(Register, XYZW);
		return *this;
	}

	inline PersistentVectorRegister4Double& operator=(const VectorRegister4Float& Register)
	{
		VectorStore(VectorRegister4Double(Register), XYZW);
		return *this;
	}

	UE_FORCEINLINE_HINT operator VectorRegister4Double() const
	{
