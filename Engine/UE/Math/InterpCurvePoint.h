...
#include "UE/Math/UnrealMemory.h"
...
{
	static_assert(
		std::is_enum_v<EnumType> && std::is_convertible_v<EnumType, int>,
		"TEnumAsByte requires an unscoped enum convertible to int.");
...
public:
	...
	constexpr TEnumAsByte(EnumType _Value)
		: __Value(static_cast<uint8>(_Value))
	{
	}

	explicit constexpr TEnumAsByte(int32 _Value)
		: __Value(static_cast<uint8>(_Value))
	{
	}

	explicit constexpr TEnumAsByte(uint8 _Value)
		: __Value(_Value)
	{
	}

	[[nodiscard]] constexpr bool operator==(EnumType _Value) const
	{
		return GetValue() == _Value;
	}

	[[nodiscard]] constexpr bool operator==(TEnumAsByte _Value) const
	{
		return __Value == _Value.__Value;
	}
	...
	{
		return static_cast<EnumType>(__Value);
	}
...
public:
	 ...
	 */
	[[nodiscard]] explicit UE_FORCEINLINE_HINT FInterpCurvePoint(EForceInit);

public:

	/** @return true if the key value is using a curve interp mode, otherwise false */
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsCurveKey() const;
...
template< class T >
inline FInterpCurvePoint<T>::FInterpCurvePoint(EForceInit)
{
	InVal = 0.0f;
