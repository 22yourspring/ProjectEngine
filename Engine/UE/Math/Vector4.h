...
public:
	...
	template<typename ColorType>
		requires std::is_same_v<std::remove_cvref_t<ColorType>, FLinearColor>
	[[nodiscard]] inline TVector4(const ColorType& InColor)
		: X(InColor.R)
		, Y(InColor.G)
		, Z(InColor.B)
		, W(InColor.A)
	...
	template<typename ColorType>
		requires std::is_same_v<std::remove_cvref_t<ColorType>, FLinearColor>
	[[nodiscard]] inline TVector4(const ColorType& InColor, T InOverrideW)
		: X(InColor.R)
		, Y(InColor.G)
		, Z(InColor.B)
		, W(InOverrideW)
...
public:
	...
	{
		const std::size_t H0 = std::hash<T>{}(Vector.X);
		const std::size_t H1 = std::hash<T>{}(Vector.Y);
		const std::size_t H2 = std::hash<T>{}(Vector.Z);
		const std::size_t H3 = std::hash<T>{}(Vector.W);
		return static_cast<uint32>(H0 ^ (H1 << 1) ^ (H2 << 2) ^ (H3 << 3));
	}
};
...
inline bool TVector4<T>::InitFromString(const std::string& InSourceString)
{
	...
	const bool bSuccessful = ParseValue(InSourceString, "X=", X) && ParseValue(InSourceString, "Y=", Y) && ParseValue(InSourceString, "Z=", Z);
