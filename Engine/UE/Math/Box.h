...
public:
	 ...
	 * @param Points Array of Points to create for the bounding volume.
	 */
	explicit TBox(const std::vector<TVector<T>>& Points)
		: TBox<T>(Points.data(), static_cast<int32>(Points.size()))
	{};
...
uint32 GetTypeHash(const TBox<T>& Box)
{
	...
	const std::size_t MinHash = HashVector(Box.Min);
	const std::size_t MaxHash = HashVector(Box.Max);
	return static_cast<uint32>(MinHash ^ (MaxHash + 0x9e3779b9u + (MinHash << 6) + (MinHash >> 2)));
}
} // namespace UE::Math
