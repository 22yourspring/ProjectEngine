...
#include "UE/Math/RotationMatrix.h"
...
public:
	 ...
	 * @param _Name The name value from which the stream will be initialized.
	 */
	FRandomStream(const FName& _Name)
	{
		Initialize(_Name);
	}
...
public:
	 ...
	 * @param _Name The name value from which the stream will be initialized.
	 */
	void Initialize(const FName& _Name)
	{
		if (_Name != NAME_None)
		{
			InitialSeed = static_cast<int32>(std::hash<FName>{}(_Name));
		}
		else
		{
			InitialSeed = static_cast<int32>(
	 ...
	 * @return A random number in [0..A)
	 */
	UE_FORCEINLINE_HINT int64 RandHelper64(int64 A) const
	{
		...
		return A > 0 ? FMath::Min<int64>(FMath::TruncToInt(GetFraction() * float(A)), A - 1) : 0;
	}
	 ...
	 */
	UE_FORCEINLINE_HINT bool RandBool() const
	{
	...
	inline FVector RandPointInBox(const FBox& Box) const
	{
