...
public:
	...
	[[nodiscard]] std::tuple<T, T, T> ToLUFEuler() const;

	// Converts to euler angles (in rads) in verse conventions
	[[nodiscard]] static TQuat MakeFromLUFEuler(const std::tuple<T, T, T>& InLUFEuler);
...
inline bool TQuat<T>::InitFromString(const std::string& InSourceString)
{
	...
	const bool bSuccess
		=  ParseValue(InSourceString, "X=", X)
		&& ParseValue(InSourceString, "Y=", Y)
		&& ParseValue(InSourceString, "Z=", Z)
		&& ParseValue(InSourceString, "W=", W);
	DiagnosticCheckNaN();
	return bSuccess;
}
...
{
	const std::size_t XHash = std::hash<T>{}(Quat.X);
	const std::size_t YHash = std::hash<T>{}(Quat.Y);
	const std::size_t ZHash = std::hash<T>{}(Quat.Z);
	const std::size_t WHash = std::hash<T>{}(Quat.W);
	return static_cast<uint32>(XHash ^ (YHash << 1) ^ (ZHash << 2) ^ (WHash << 3));
}

template<typename T>
[[nodiscard]] std::tuple<T, T, T> TQuat<T>::ToLUFEuler() const
{
...
template<typename T>
[[nodiscard]] TQuat<T> TQuat<T>::MakeFromLUFEuler(const std::tuple<T, T, T>& InLUFEuler)
{
	const T LeftAxisAngle = std::get<0>(InLUFEuler);
	const T UpAxisAngle = std::get<1>(InLUFEuler);
	const T ForwardAxisAngle = std::get<2>(InLUFEuler);
