...
inline std::string TRotator<T>::ToCompactString() const
{
	...
	{
		ReturnString += StringFormat("P=%.2f", Pitch);
...
inline bool TRotator<T>::InitFromString( const std::string& InSourceString )
{
	...
	const bool bSuccessful = ParseValue(InSourceString, "P=", Pitch ) && ParseValue(InSourceString, "Y=", Yaw ) && ParseValue(InSourceString, "R=", Roll );
	DiagnosticCheckNaN();
	return bSuccessful;
}
