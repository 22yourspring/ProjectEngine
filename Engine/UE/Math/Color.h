...
struct FLinearColor
{
	...
	*/
	 ...
	 * @return					Path length.
	 */
	[[nodiscard]] static CORE_API float EvaluateBezier(const FLinearColor* ControlPoints, int32 NumPoints, std::vector<FLinearColor>& OutPoints);
	...
	*/
	...
	{
		const std::size_t RHash = std::hash<float>{}(LinearColor.R);
		const std::size_t GHash = std::hash<float>{}(LinearColor.G);
		const std::size_t BHash = std::hash<float>{}(LinearColor.B);
		const std::size_t AHash = std::hash<float>{}(LinearColor.A);
		return static_cast<uint32>(RHash ^ (GHash << 1) ^ (BHash << 2) ^ (AHash << 3));
	}
};
