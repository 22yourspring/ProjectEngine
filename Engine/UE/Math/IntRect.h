...
struct TIntRect
{
	 ...
	 * Constructor
	 *
	 * @param X0 Minimum X coordinate (inclusive)
	 * @param Y0 Minimum Y coordinate (inclusive)
	 * @param X1 Maximum X coordinate (exclusive)
	 * @param Y1 Maximum Y coordinate (exclusive)
	 * 
	 * @note  
	 */
	 ...
	 * Constructor
	 *
	 * @param InMin Minimum Point (inclusive)
	 * @param InMax Maximum Point (exclusive)
	 */
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	 ...
	 */
	[[nodiscard]] bool Contains(const FIntRect& Other) const
	{
	 ...
	 */
	[[nodiscard]] static constexpr int32 Num()
	{
		return 2;
	}
...
{
	const std::size_t MinXHash = std::hash<IntType>{}(InRect.Min.X);
	const std::size_t MinYHash = std::hash<IntType>{}(InRect.Min.Y);
	const std::size_t MaxXHash = std::hash<IntType>{}(InRect.Max.X);
	const std::size_t MaxYHash = std::hash<IntType>{}(InRect.Max.Y);
	return static_cast<uint32>(MinXHash ^ (MinYHash << 1) ^ (MaxXHash << 2) ^ (MaxYHash << 3));
}
