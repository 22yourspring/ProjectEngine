...
#include "UE/Math/RotationTranslationMatrix.h"
#include "UE/Math/QuatRotationTranslationMatrix.h"
...
public:
	 ...
	 * @param Rot rotation
	 */
	[[nodiscard]] TRotationMatrix(const TRotator<T>& Rot)
	...
	[[nodiscard]] static TMatrix<T> Make(TRotator<T> const& Rot)
	{
		return TRotationMatrix(Rot);
	}

	/** Matrix factory. Return an TMatrix<T> so we don't have type conversion issues in expressions. */
	[[nodiscard]] static TMatrix<T> Make(TQuat<T> const& Rot);

	/** Builds a rotation matrix given only a XAxis. Y and Z are unspecified but will be orthonormal. XAxis need not be normalized. */
	[[nodiscard]] static TMatrix<T> MakeFromX(TVector<T> const& XAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromZ(TVector<T> const& ZAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromXY(TVector<T> const& XAxis, TVector<T> const& YAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromXZ(TVector<T> const& XAxis, TVector<T> const& ZAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromYX(TVector<T> const& YAxis, TVector<T> const& XAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromYZ(TVector<T> const& YAxis, TVector<T> const& ZAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromZX(TVector<T> const& ZAxis, TVector<T> const& XAxis);
	...
	[[nodiscard]] static TMatrix<T> MakeFromZY(TVector<T> const& ZAxis, TVector<T> const& YAxis);
};
